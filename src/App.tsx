...
function App() {
  ...
  useEffect(() => {
    const i60 = setInterval(() => dispatch(setTimer60()), 50_000);
    const i5 = setInterval(() => dispatch(setTimer5()), 5_000);
    return () => {
      clearInterval(i60);
      clearInterval(i5);
    };
  ...
  useEffect(() => {
    if (window.location.protocol !== 'https:' && window.https === true) {
      ...
      window.location.href = window.location.href.replace(/^https:\/\//i, 'http://');
    }
  }, []);
